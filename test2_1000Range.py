...
def main():
	...
	for numberOfJobs in range(1, 11):
		for numberOfFunctionCalls in range(0, 50001, 10000):

			startTime = time.monotonic()

			# print('starting: number of jobs: ' + str(numberOfJobs) + ' functionCalls = ' + str(numberOfFunctionCalls))

			results = joblib.Parallel(n_jobs = numberOfJobs)(joblib.delayed(double)(i, numberOfJobs, numberOfFunctionCalls) for i in range(10000))
	...
	plt.title('Parallel Python With Joblib')
	plt.show()
